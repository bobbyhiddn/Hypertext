#!/usr/bin/env python3
"""One-shot, manual Gemini regeneration of the six shared Lot reference faces."""
...
def prompt(cards: int, role: str, title: str, recipe: str, reward: str) -> str:
    role_label = f"{role.upper()} LOT"
    slot_instruction = {
...
def main() -> None:
    ...
    for cards, role, title, recipe, reward in CASES:
        ...
        records.append({
            "cards": cards, "role": role, "title": title, "recipe": recipe,
            "reward": reward, "path": str(target.relative_to(ROOT)), "sha256": sha(target),
            "generation_record": str((directory / "generation.json").relative_to(ROOT)),
            "generation_record_sha256": sha(directory / "generation.json"),
            "prompt": prompt(cards, role, title, recipe, reward),
        })
    ...
    if len(records) != 6:
        records = []
        for cards, role, title, recipe, reward in CASES:
            target = OUT / f"{cards}-card/{role}_template_1024x1536.png"
            records.append({
                "cards": cards, "role": role, "title": title, "recipe": recipe,
                "reward": reward, "path": str(target.relative_to(ROOT)), "sha256": sha(target),
    ...
    manifest = {
        ...
        "family": "Lot",
        "requirements": ["REQ-PPAUG-020", "REQ-PPAUG-025"],
