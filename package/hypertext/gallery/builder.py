#!/usr/bin/env python3
"""Build static gallery site for Hypertext cards."""
...
def build_gallery(
    ...
    *,
    ...
    """
    # Validate inputs
    for p in [index_template, series_template, assets_css, assets_js, rules_template]:
        if not p.exists():
            print(f"Missing required gallery file: {p}")
            return 1

    series_dirs = _find_series_dirs(series_dir)
    if not series_dirs:
        print(f"No series directories found under: {series_dir}")
        return 1
    ...
    _copy_assets(out_dir=out_dir, css_src=assets_css, js_src=assets_js)
    ...
    for series_path in series_dirs:
        ...
        series_out_dir = out_dir / "series" / series_name
        ...
        template = _read_text(series_template)
        series_page = (
            template.replace("{SERIES_NAME}", series_name)
    ...
    template = _read_text(index_template)
    index_page = (
        template.replace("<!-- SERIES_INJECTION_POINT -->", "\n".join(index_series_cards))
        .replace("{GENERATION_DATE}", generation_date)
    )
    _write_text(out_dir / "index.html", index_page)

    # Convenience /series/ index (redirect to root)
    _write_text(
        out_dir / "series" / "index.html",
        "<!doctype html><meta charset=\"utf-8\"><meta http-equiv=\"refresh\" content=\"0; url=../index.html\">",
    )

    # Build rules page
    _build_rules_page(
        rules_md_path=rules_md,
        template_path=rules_template,
        out_dir=out_dir,
        generation_date=generation_date
    )

    print(f"Gallery built at {out_dir / 'index.html'} with {len(series_dirs)} series")
    return 0
...
def main():
    ...
    return build_gallery(
        ...
        series_template=args.series_template,
        rules_template=args.rules_template,
        rules_md=args.rules_md,
        assets_css=args.assets_css,
        assets_js=args.assets_js,
    )
