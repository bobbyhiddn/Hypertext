#!/usr/bin/env python3
"""Core watermark cryptographic functions for Hypertext cards."""
...
def load_card_identity(card_dir: Path) -> dict:
    """Load identity fields from card.json."""
...
def canonical_payload(identity: dict) -> str:
...
def compute_signature_hex(payload: str) -> str:
    """Compute HMAC-SHA256 signature hex of payload."""
    key = _get_key()
...
def signature_bits(sig_hex: str, bit_count: int) -> list[int]:
    """Convert hex signature to bits MSB first."""
...
def build_svg(*, sig_hex: str, payload: str, size_px: int = 72) -> str:
    """Build SVG watermark from signature."""
    # 5x5 grid = 25 bits
...
_load_card_identity = load_card_identity
