#!/usr/bin/env python3
"""Image utility functions for Hypertext."""
...
def convert_jpeg_to_png(path: Path | str, keep_original: bool = False) -> int:
    """Convert JPEG images to PNG format.

    Args:
        path: File or directory path to convert.
        keep_original: If True, keep original JPEG files.

    Returns:
        Number of files converted (or 0 on error for single file).
    """
    if Image is None:
    ...
    if path.is_file():
        result = _convert_file(path, keep_original)
        return 1 if result else 0
    elif path.is_dir():
        return _convert_directory(path, keep_original)
    else:
        print(f"Path not found: {path}", file=sys.stderr)
        return 0


def _convert_file(jpeg_path: Path, keep_original: bool = False) -> Path | None:
...
def _convert_directory(dir_path: Path, keep_original: bool = False) -> int:
    ...
    for ext in ("*.jpeg", "*.jpg", "*.JPEG", "*.JPG"):
        for jpeg_path in dir_path.glob(ext):
            if _convert_file(jpeg_path, keep_original):
                converted += 1
...
def main() -> int:
    """CLI entrypoint for JPEG to PNG conversion."""
    ...
    if path.is_file():
        result = _convert_file(path, args.keep)
        return 0 if result else 1
    elif path.is_dir():
        count = _convert_directory(path, args.keep)
        if count == 0:
