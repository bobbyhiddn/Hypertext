...
def content():
    return {
        "NUMBER": 7, "CARD_TYPE": "NOUN", "RARITY": "RARE", "WORD": "חֶסֶד — χάρις",
        ...
        "TRIVIA_BULLETS": ["First—exact.", "Second: exact.", "Third & exact."], "SERIES": "2026-Q1",
    }
...
def test_invalid_enums_are_rejected(content, card_type, rarity, mode):
    content.update(CARD_TYPE=card_type, RARITY=rarity)
...
def test_generated_watermark_is_always_forbidden(content):
    for card_type in ("NOUN", "VERB", "ADJECTIVE", "NAME", "TITLE"):
        for rarity in ("COMMON", "UNCOMMON", "RARE", "GLORIOUS"):
            item = dict(content, CARD_TYPE=card_type, RARITY=rarity)
