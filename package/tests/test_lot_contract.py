...
def test_source_subtype_prompt_and_reference_agree(tmp_path, cards, points, letters):
    ...
    manifest = json.loads((ROOT / "templates/lot/v002/manifest.json").read_text())
    entry = next(x for x in manifest["templates"] if x["cards"] == cards)
    assert f"CHAPTER VALUE: {points} POINTS" in entry["visible_strings"]
    split = f'{entry["visitor_letters"]}/{entry["owner_letters"]}'
    assert f"PORTION VALUE: {split} LETTERS" in entry["visible_strings"]
    assert entry["owner_letters"] == letters
    import hashlib as _hashlib
    assert _hashlib.sha256((ROOT / entry["path"]).read_bytes()).hexdigest() == entry["sha256"]
